...
function StudentPreview({
  ...
  children?: React.ReactNode;
    ...
    const amountInPaise = (Number.isFinite(pricing.price) ? pricing.price : 0) * 100;

const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/order/razorpay/create-order`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/order/razorpay/verify`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
...
<button
  onClick={pay}
  className="mt-3 w-full bg-blue-600 hover:bg-blue-700 text-white rounded-md py-2"
>
  Buy now
</button>            
            </div>
